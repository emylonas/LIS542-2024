...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
    ...
    <pattern>
        <rule context="tei:div/@n">
            <assert test="matches(., 'poem\d+$')">The n attribute of div elements must be in the
                form of poem0000. </assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:l">
            <assert test="string-length(@n) = 4">The n attribute of the l element must contain four digits.</assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:revisionDesc/tei:change">
            <assert test="string-length() != 0"> The change element within the RevisionDesc element must contain a string of one or more characters. </assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:div">
            <assert test="(@type = 'sonnet') or (@type = 'limerick')">The type attribute in the div element must either be 'sonnet' or 'limerick'.</assert>
        </rule>
    </pattern>
    ...
    <pattern> 
        <rule context="tei:l[ancestor::tei:div[@type='sonnet']]">
            <assert test="@rhyme">Each line in a poem with the type attribute 'sonnet' must have a rhyme attribute.</assert>
        </rule> 
    </pattern>
...
<pattern>
    <rule context="tei:l">
        <assert test="tei:lb[1]">All l elements must immediately be followed with an lb element.</assert>
    </rule>
</pattern>
   ...
   -->
<!--CG: Check if each poem has a title.-->
    <pattern>
    <rule context="tei:head">
        <assert test="string-length() != 0">All head elements must contain a character string representative of the poem's title and author.</assert>
    </rule>
    </pattern>

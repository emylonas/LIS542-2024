...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
    ...
    <pattern>
        <rule context="tei:div">
            <assert test="starts-with(@n, 'poem')"> Flag the "n" attribute on the "div" element if
                its value doesn't begin with the letters "poem." </assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:l">
            <assert test="string-length(@n) = 4"> "n" must have four digits. </assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:RevisionDesc/tei:change">
            <assert test="string-length() ne 0"> The "change" element in the "RevisionDesc" element
                cannot be empty. </assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:div">
            <assert test="@type = 'sonnet' or @type = 'limerick'"> Check that the @type attribute
                within the "div" element is one of either "sonnet" or "limerick." </assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="tei:l[ancestor::tei:div[@type = 'sonnet']]">
            <assert test="@rhyme"> All line in each sonnet must contain a rhyme attribute. </assert>
        </rule>
    </pattern>
    <!-- Question here: is there a way to "disable" a rule without deleting the whole thing? A way to tell Oxygen or XML to ignore this rule? -->
   ...
   -->
    ...
    <pattern>
        <rule context="tei:l">
            <assert test="tei:lb[1]">
                Each "l" element must immediately be followed by an "lb" element.
            </assert>
        </rule>
    </pattern>
   ...
   -->
    ...
    <pattern>
        <rule context="tei:lg[ancestor::tei:div[@type='sonnet']]">
            <assert test="count(tei:l) = 14">
                Sonnets must have exactly 14 lines.
            </assert>
        </rule>
    </pattern>

...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
    ...
    <pattern>
      <rule context="l">
          <assert test="lb">This line has to have a linebreak element!</assert>
      </rule>
    </pattern>
    ...
    <pattern>
        <rule context='div'>
            <assert test="starts-with(@n, 'poem')">This attribute has to start with "poem"!</assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context='l'>
            <assert test="starts-with(@n, '000') or starts-with(@n, '00')">This attribute has to be formatted as four digits (use leading zeros)!</assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="RevisionDesc">
            <assert test="string-length(change) != 0">This element requires text!</assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="div">
            <assert test="(@type = 'sonnet') or (@type = 'limerick')">This element can only be of required type "sonnet" or "limerick"!</assert>
        </rule>
    </pattern>
    ...
    <pattern>
        ...
        <rule context='lg'>
            <assert test="descendant::l[@rhyme]">This element has to have a rhyme attribute since it's a sonnet!</assert>
        </rule>
    </pattern>
   ...
   -->
    ...
    <pattern>
        <rule context="l">
            <assert test="lb[1]">Line elements must always be followed by linebreak elements!</assert>
        </rule>
    </pattern>

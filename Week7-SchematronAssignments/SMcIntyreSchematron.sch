...
<pattern>
    <rule context="tei:l">
        <assert test="tei:lb">l does not have an lb element inside it</assert>
    </rule>
    ...
    <pattern>
        <rule context="tei:div">
            <assert test="starts-with(@n, 'poem')">Div attribute n does not start with poem</assert>
        </rule>
    ...
    <pattern>
        <rule context="tei:l">
            <assert test="string-length(@n) = 4">n attribute does not contain 4 digits</assert>
        </rule>
    ...
    <pattern>
        <rule context="tei:revisionDesc/tei:change">
            <assert test="string-length() ne 0">change element cannot be empty</assert>
        </rule>
    ...
    <pattern>
        <rule context="tei:div/@type">
            <assert test="(.='sonnet') or (.='limerick')">type should be either sonnet or limerick</assert>
        </rule>
    ...
    <pattern>
        <!-- Sophie: ran out of time, didn't finish -->

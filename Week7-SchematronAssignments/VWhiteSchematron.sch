...
<pattern>
  <rule context="tei:l">
      <assert test="tei:lb">there must be an lb element inside a l element.</assert>
  </rule>  
</pattern>
    ...
    <pattern>
        <rule context="tei:div">
            <assert test="starts-with(@n, 'poem')"> the @n attribute on div must begin with "poem"</assert>
        </rule> 
    </pattern>
    ...
    <pattern>
        <rule context="tei:l">
            <assert test="string-length(@n) =4" > line numbers must contain 4 digits</assert>
        </rule>       
    </pattern>
    ...
    <pattern>
      <rule context="tei:revisionDesc/tei:change">
          <assert test="string-length()=0" >there must be content in the change element</assert>
      </rule>  
    </pattern>
    ...
    <pattern>
       <rule context="tei:div">
           <assert test="(@type='sonnet') or (@type='limerick')">the type attribute on the div element must have either "sonnet" or "limerick".</assert>
       </rule> 
    </pattern>
    ...
    <pattern>
       <rule context="tei:l[ancestor::tei:div[@type='sonnet']]">
           <assert test="@rhyme">the lines in a sonnet must have a rhyme attribute</assert>   
       </rule>
    </pattern>
   ...
   -->
    ...
    <pattern>
        <rule context="tei:l">
            <assert test="tei:lb[1]">each line in a poem must be immediately followed by an lb element</assert>
        </rule>
    </pattern>
   ...
   <!-- 
       ...
       I want to write a rule to flag when a <head> element in <div> has no content.
   -->
   
    <pattern>
        <rule context="tei:div/tei:head">
            <assert test="string-length()=0" >there must be content in the head element</assert>
        </rule>       
    </pattern>

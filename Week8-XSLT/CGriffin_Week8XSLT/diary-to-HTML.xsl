...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                    ...
                    <p>
                        <ol>                          
                             <xsl:for-each select="//d:entries//d:name" xml:space="preserve">
                                                                <li>
                                                                      <xsl:value-of select="."/> <xsl:if test="@role">
    ...
    <xsl:template match="d:p">
        ...
        <p> [<xsl:value-of select="replace(@n, 'p0?', 'p. ')"/>] <xsl:apply-templates/>
        </p>
    </xsl:template>
    ...
    <xsl:template match="d:cite">
        <a href="https://www.kingjamesbibleonline.org/Titus-3-5/">
            ...
            <!--CG: Looked up a way to open the tab in a new tab for number 5 in the assignment, which I think worked I think :) -->
            <xsl:apply-templates/>
        </a>
    </xsl:template>
    ...
    <xsl:template match="d:q"> “<xsl:apply-templates/>” </xsl:template>
    <!--CG: Added quotations around quote based on how pb is done above and assignment hints.-->

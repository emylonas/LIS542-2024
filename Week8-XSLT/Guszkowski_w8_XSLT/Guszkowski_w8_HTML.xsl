...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                    ...
                    <p>
                        <ol>
                                                       <xsl:for-each select="//d:entries//d:name" xml:space="preserve">
                                                                <li>
                                                                      <xsl:value-of select="."/>
                                                                       <xsl:value-of select="concat(', ',translate(@role, '', ''))"/> <!-- add role to name list -->
                                                                    </li>
                                                            </xsl:for-each>
                        </ol>
                    </p>
    ...
    <xsl:template match="d:pb"> [<xsl:value-of select="@n"/>] 
        <xsl:apply-templates/>
    </xsl:template>
    <!--  or  select="concat('page ',translate(@n, 'page0', ''))"  for a more elegant display -->
   
    <xsl:template match="d:p"> [<xsl:value-of select="concat('p',translate(@n, 'p0', ''))"/>]  <!-- add paragraph numbers -->
        <xsl:apply-templates/>
    </xsl:template>
   
    <xsl:template match="d:q">  <!-- surround quote in quotation marks -->
        "<xsl:apply-templates/>"
    </xsl:template>
    
    <xsl:template match="d:cite"> [<xsl:value-of select="@ref"/>] 
        <a href="{@ref}">
        <xsl:apply-templates/>
        </a>
    </xsl:template>

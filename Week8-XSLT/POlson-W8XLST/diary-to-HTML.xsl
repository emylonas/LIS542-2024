                    ...
                    <p>
                        <ol>
                                                       <xsl:for-each select="//d:entries//d:name" xml:space="preserve">
                                                                <li>
                                    <!-- add honorific titles in parenthesis before names -->
                                                                      (<xsl:value-of select="@Honorific"/>) <xsl:value-of select="."/> (<xsl:value-of select="@role"/>) 
                                                                    </li>
                                                            </xsl:for-each>
                        </ol>
                    </p>
...
<!-- display paragraph numbers -->    
    <xsl:template match="d:p"> 
        <xsl:choose>
            <xsl:when test="starts-with(@n,'p0')"> 
                [<xsl:value-of select="substring-after(@n, 'p0')"/>]
            </xsl:when>
            <xsl:otherwise>
...
<!-- display quotes around the biblical quotation. schema was changed to separate the citation from the quote in order to prevent quote marks including citation -->
    <xsl:template match="d:quote"> 
        &quot;<xsl:apply-templates/>&quot;
    </xsl:template>
    
<!-- provide link to citation location -->
    <xsl:template match="d:cite">
        <a href="@ref">
        <xsl:apply-templates/>
        </a>

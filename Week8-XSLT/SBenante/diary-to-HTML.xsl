...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <html>
            ...
            <body>
                ...
                <div>
                    <h1>List of Deletions</h1>
                    <p>
                        <ol>
                            <xsl:for-each select="//d:entries//d:del" xml:space="preserve">
                                                                <li>
                                                                      <xsl:value-of select="."/> (<xsl:value-of/>)
                                                                    </li>
                                                            </xsl:for-each>
    ...
    <xsl:template match="d:p"> <!-- Match <entry>, surround each one with an HTML <p> element and keep going  [<xsl:value-of select="concat('paragraph ',translate(@n, 'paragraph0', ''))"/>] </xsl:template>-->
        <p>
            <xsl:apply-templates/> 
        </p>
    </xsl:template>
    <xsl:template match="d:q">"Not by Works of Righteousness, which We have done, but according to his Mercy he saved us, through the Washing of Regeneration and the Renewing of the holy Ghost."</xsl:template>
    ...
    <!-- <xsl:template select="concat('page',translate(@n, 'page0, ''))"></xsl:template> -->

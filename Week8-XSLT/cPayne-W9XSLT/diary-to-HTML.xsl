...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <html>
            ...
            <body>
                    ...
                    <p>
                        <ol>
                                                       <xsl:for-each select="//d:entries//d:name" xml:space="preserve">
                                                                <li>
                                                                      <a href="#{generate-id()}"><xsl:value-of select="."/> <xsl:if test="@role">(<xsl:value-of select="@role"/>)</xsl:if></a>
                                                                    </li>
                                                            </xsl:for-each>
                        </ol>
                    </p>
                ...
                <div>
                    ...
                    <p>
                        <ol>
                                                       <xsl:for-each select="//d:place" xml:space="preserve">
                                                                <li>
    ...
    <xsl:template match="d:p"> <!-- Match <entry>, surround each one with an HTML <p> element and keep going  -->
        <p>
            [<xsl:value-of select="number(substring-after(@n, 'p'))"/>]
            <xsl:apply-templates/>
        </p>
    </xsl:template>
    ...
    <xsl:template match="d:pb"> [<xsl:value-of select="concat('Page ', number(substring-after(@n, 'page')))"/>] </xsl:template>

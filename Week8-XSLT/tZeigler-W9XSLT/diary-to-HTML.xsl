...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="d:pb"> [<xsl:value-of select="@n"/>] </xsl:template>
     ...
     <xsl:template match="d:cite"> "<xsl:apply-templates/>"</xsl:template> 
    
    <!--  or  select="concat('page ',translate(@n, 'page0', ''))"  for a more elegant display -->

<!-- I am now finally attempting to do something below to effect the requested modifications...I am eternally reluctant to risk breaking things that currently function.  
    starting with the paragraph number capture and mimicking the page break/page number capture-->
    
    <!--     <xsl:template match="d:p">[<xsl:value-of select="@n"/>]</xsl:template>   -->
    
    
    <!--   Mmmmm... i unleashed this on the Adams file and it gave me bracketed paragraph numbers but deleted all the text content of entries...  Not likely the goal.  I'm missing something big here  --> 
    
    
    <!--      <xsl:template match="d:p">[<xsl:value-of select="concat('p ',translate(@n, 'paragraph0',"))"/>] </xsl:template>    -->
    <!-- The above was an even bigger mess as it keep calling for a / closure to follow value-of  select...? -->

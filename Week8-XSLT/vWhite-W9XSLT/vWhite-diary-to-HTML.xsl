...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="d:p"> [<xsl:value-of select="concat('p', translate(@n,'p0', ' '))"/>] <!-- paragraph numbers -->
        <xsl:apply-templates/>            
    </xsl:template>

    <xsl:template match="d:q"> <!-- quotation marks -->
        "<xsl:apply-templates/>"
    </xsl:template>
    
    <xsl:template match="d:cite"> 
        <a href="https://www.kingjamesbibleonline.org/Titus-3-5/"> link text <xsl:apply-templates/> </a>
    </xsl:template>

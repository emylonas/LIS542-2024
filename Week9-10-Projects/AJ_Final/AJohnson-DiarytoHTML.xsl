...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="d:pb"> 
        <pb>
            [<xsl:value-of select="replace(@n, 'pb00?', 'page break  ')"/>]   
        </pb>
    </xsl:template>
...
<!-- add citation link to quotes -->
    <xsl:template match="d:quote">
        <a href="{@cite}"> <xsl:apply-templates/> </a>
    </xsl:template>

...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <html>
            ...
            <body>
                ...
                <div>
                    ...
                    <p>
                        <ol>
                                                       <xsl:for-each select="//d:entries//d:weather" xml:space="preserve">
                                                                <xsl:sort select="." order="descending"/>
                                                                <li>
                                                                      <xsl:value-of select="."/> 
                                                                    </li>
    ...
    <xsl:template match="d:weather"> <!-- Enclosing in HTML span element with class attribute so CSS formatting can be applied. -->
        <span class="weather"><em><xsl:apply-templates/></em></span>
    </xsl:template>
    
    <xsl:template match="d:media"> <!-- Enclosing in HTML span element with class attribute so CSS formatting can be applied. -->
        <span class="media"><xsl:apply-templates/></span>
    </xsl:template>
    ...
    <xsl:template match="d:cs">
        <span class="cs"><em><xsl:apply-templates/></em></span>
        
    </xsl:template>
    
    <xsl:template match="d:quote">
        <span class="quote"><em><xsl:apply-templates/></em></span>
        
    </xsl:template>
    
    <xsl:template match="d:cw">
        <a href="https://en.wikipedia.org/wiki/Trauma_trigger">
            <xsl:apply-templates/>
        </a>
    </xsl:template>
    <xsl:template match="d:cite">
        <a href="https://www.kingjamesbibleonline.org/Titus-3-5/">
            <xsl:apply-templates/>
        </a>
    </xsl:template>
    <xsl:template match="d:pb"> <h3>[<xsl:value-of select="replace(@n, 'p0', '')"/>] </h3> </xsl:template>
    ...
    <!-- <xsl:template match='*'>
        ...
        QQQ-element: <xsl:value-of select="place()"/>

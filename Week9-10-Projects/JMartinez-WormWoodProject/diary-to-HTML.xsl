...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <html>
            ...
            <body>
                ...
                <div>
                    ...
                    <p>
                        <ol>
                            <xsl:for-each select="//d:entries//d:abbr" xml:space="preserve">
                                                                <li>
                                                                      <xsl:value-of select="."/> (<xsl:value-of select="following-sibling::d:expan"/>)
                                                                    </li>
                                                            </xsl:for-each>
                        </ol>
                    </p>
    ...
    <xsl:template match="d:p"> <!-- Match <entry>, surround each one with an HTML <p> element and keep going  -->
        <p>
            [<xsl:value-of select="concat('paragraph ', translate(@id, 'p0',''))"/>]
            <xsl:apply-templates/>
        </p>
    </xsl:template>
    ...
    <xsl:template match="d:entries/d:date">
    ...
    <xsl:template match="d:act">
        <xsl:apply-templates/>
    </xsl:template>

    <xsl:template match="d:marginNote">
        <xsl:apply-templates/>
    </xsl:template>
    ...
    <xsl:template match="d:pb"> <p><xsl:apply-templates/></p> </xsl:template>

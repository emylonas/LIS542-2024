...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                    ...
                    <p>
                        <ol>                          
                             <xsl:for-each select="//d:entries//d:name" xml:space="preserve">
                                <li>
                                   <xsl:value-of select="."/> (<xsl:value-of select="@gender"/>)
                                </li>
                            </xsl:for-each>
                        </ol>
                    </p>
    ...
    <xsl:template match="d:name">
        <span>
            <xsl:attribute name="style">
                <xsl:choose>
                    <xsl:when test="@gender = 'male'">color: blue;</xsl:when>
                    <xsl:when test="@gender = 'female'">color: red;</xsl:when>
                    <xsl:when test="@gender = 'other'">color: green;</xsl:when>
                    <xsl:when test="@gender = 'unknown'">color: yellow;</xsl:when>
                </xsl:choose>
            </xsl:attribute>
            <xsl:apply-templates/>
        </span>
    ...
    <xsl:template match="d:place">
        <!-- Enclosing in HTML a element with href attribute so it will appear as a hyperlink with mapRef as URL destination. -->
        <!-- after discussion with Prof. Elli, this isn't the ideal way to do this (URL could get changed quite a bit resulting in unscalable upkeep) but for the sake of the final project as practice will suffice -->
        <a>

...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="d:name"> <!-- Enclosing in HTML span element with class attribute so CSS formatting can be applied. -->
            <span style="{if (@role = 'student') then 'color: green;' else if (@role = 'friend') then 'color: blue;' else if (@role = 'teacher') then 'color: red;' else ''}">
                <xsl:apply-templates/>
            </span>
    </xsl:template>
    ...
    <xsl:template match="d:quote"> 
        <q><xsl:apply-templates/></q> <!-- adds quotation marks around quotes. -->
    </xsl:template>
